...
export default function BillingPage() {
  const b: BillingData = billing as BillingData;
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={120}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ background: "#111827", border: "1px solid #374151", borderRadius: 8, fontSize: 11 }} formatter={(v: ValueType | undefined) => [`$${Number(Array.isArray(v) ? v[0] : (v ?? 0)).toFixed(2)}`, ""]} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={last14}>
            ...
            <Tooltip contentStyle={{ background: "#111827", border: "1px solid #374151", borderRadius: 8 }} labelStyle={{ color: "#d1d5db" }} itemStyle={{ color: "#a78bfa" }} formatter={(v: ValueType | undefined) => [`$${Number(Array.isArray(v) ? v[0] : (v ?? 0)).toFixed(2)}`, "Spend"]} />
